...
export const AILeadModal: React.FC<AILeadModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleUpdateExisting = () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const { id, workspaceId: wId, historico: newHist, createdAt, updatedAt, ...leadData } = AIService.convertToLead(analysis, workspaceId) as Lead;
    ...
    addToast({ type: 'success', message: 'Lead existente atualizado com sucesso!' });
    onClose();
    resetState();
  };
      ...
      <Card className="w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl border-slate-200">
        <div className="flex items-center justify-between p-4 border-b bg-slate-50">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-800 uppercase tracking-tight">Criar Lead com IA</h2>
              <p className="text-xs text-slate-500 font-medium">Extraia dados de conversas do WhatsApp automaticamente</p>
            </div>
          </div>
          ...
          {!analysis ? (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2 text-slate-500 text-sm italic">
                ...
                <span>Dica: Copie toda a conversa do WhatsApp e cole aqui.</span>
              </div>
            </div>
          ) : (
        ...
        <div className="p-4 border-t bg-slate-50 flex justify-end gap-3">
          {!analysis ? (
            <>
              <Button variant="ghost" onClick={onClose} disabled={isAnalyzing}>Cancelar</Button>
              <Button 
            ...
            <>
              <Button variant="ghost" onClick={() => setAnalysis(null)}>Recomeçar</Button>
              {duplicateLead ? (
                <Button variant="secondary" onClick={handleUpdateExisting} className="gap-2">
              ...
              {duplicateLead && (
                <Button variant="ghost" onClick={handleCreateNew} className="gap-2">
